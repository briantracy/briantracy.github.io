...
- Read 20 books, 5 of which are non fiction ([progress](/reading.html) 16 books, 2 nonfiction)
- Read a textbook
- Find a friend not related to work or school
- Complete 150 crossword puzzles (progress: 83 done) <!-- https://www.nytimes.com/puzzles/stats -->
- Submit a crossword puzzle to the NYTimes (I ended up making lots of mini crosswords)
- Write and share a short story
- Make 30 contributions to Wikipedia ([progress](https://en.wikipedia.org/wiki/Special:Contributions/CasioWatchLover) 10 contributions)
- Touch my toes while standing
- Learn how to whistle
- Learn how to juggle 3 items
- Type 120 wpm (yeah boi [typeracer account](https://data.typeracer.com/pit/profile?user=casio_watch_lover))
- Typeset a book for [Standard Ebooks](https://standardebooks.org) (progress: made [some](https://github.com/standardebooks/homer_the-odyssey_william-cullen-bryant/commit/f8baee1de7be4049176286ef29aeee82240c7ce6) [edits](https://github.com/standardebooks/homer_the-odyssey_william-cullen-bryant/commit/628b403de49daec82f855c4eff26b0bdd8622f57))
...
| Aug 5  | y   | y     | y    | w    | y      | y        | hike, bench(4x10@135), OHP(4x10@80) | |
...
| Sep 11 | y   | y     | y    | y    |        | y        | bike to work, bsquat(up to 8@170) | |
...
| Oct 18 | y   | y     | y    | y    |        | y        | bike to work, row(4x12@135, lots of extra reps) | |
...
| Nov 24 | y   |       | y    | h    | y      |          | run 5mi, baseball, tennis | Trail run at Arastradero |
...
| Dec 31 |     |       | y    |  w   |        | y        | tennis | Flight home |
...
const results = {};
for (let i = 0; i < cells.length; ++i) {
    if ((i + 1) % CELLS_PER_ROW != 0 && (i) % CELLS_PER_ROW != 0) {
        cells[i].style.backgroundColor = '#ababab'
        if (cells[i].innerHTML == '') {
            if (results[i % CELLS_PER_ROW] == undefined) {
                results[i % CELLS_PER_ROW] = 1;
            } else {
                results[i % CELLS_PER_ROW]++;
            }
            cells[i].style.backgroundColor = '#aa44bb'
            cells[i].innerHTML = results[i % CELLS_PER_ROW];
        }
    }
}
console.log(results)
